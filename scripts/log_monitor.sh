...
getStateFile(){
   ...
   then 
      ...
      echo "LastProcessed=0" > state/$updatedName
      echo "EMAIL_SENT=false" >> state/$updatedName 
   fi
...
stateFileCreation(){
    ...
    echo "EMAIL_SENT=false" >> "$STATE_FILE"
...
do
    validatingLogFile "$logFile" || continue

    stateFile=$(getStateFile $logFile)

    loadingStateFile "$stateFile" || continue
